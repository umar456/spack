...
def get_concrete_specs(env, root_spec, job_name, related_builds,
    ...
    """ Build a dictionary of concrete specs relevant to a particular
        ...
        rebuilt (which could be the same), as well as the specs of
        any dependencies.
    ...
    Arguments:
        ...
        job_name (str): Name of package to be built, used to index the
            ...
            built.
        related_builds (str): Semi-colon separated list of dependency
            package names for reporting related builds to CDash.  Also
            used to index the root spec.
...
def register_cdash_build(build_name, base_url, project, site, track):
    """ Request a build id from CDash

    Arguments:

        build_name (str): Name to give build
        base_url (str): URL of CDash instance
        project (str): Name of project
        site (str): CDash site name
        track (str): CDash track e.g. 'Experimental', 'Nightly', ...

    Returns:

        Returns a tuple of build id and build stamp.  The build stamp is
        used by "spack install" when reporting build results, the build
        id, is used to relate builds to their dependency builds in CDash.
    """
...
def relate_cdash_builds(spec_map, cdash_base_url, job_build_id, cdash_project,
    ...
    """ Relates builds to dependency builds in CDash.  The build represented by
        job_build_id is related to each of it's dependency builds.  The build ids
        of dependency builds are persisted in a binary mirror next to the spec.json
        files, and so must be retrieved from the mirror before they can be used.

    Arguments:

        spec_map (dict): The same structure produced by get_concrete_specs(), above.
        cdash_base_url (str): URL of the CDash instance
        job_build_id (str): CDash build id of the build to relate to its dep builds
        cdash_project (str): CDash project
        cdashids_mirror_urls (list): Root mirror urls to search for dependency cdash
            build ids
    """
    if not job_build_id:
        return
...
def write_cdashid_to_mirror(cdashid, spec, mirror_url):
    """ Constructs spec-specific url for cdash build id and pushes the build id
        to the mirror.

    Arguments:

        cdashid (str): CDash build id to be pushed to the mirror as a file
        spec (spack.spec.Spec): Concrete spec associated with build
        mirror_url (str): Base mirror url (not including "build_cache")
    """
...
def read_cdashid_from_mirror(spec, mirror_url):
    """ Attempt to read cdash build id for given concrete spec from
        the given mirror.  Either return the build id or raise an
        exception.

    Arguments:

        spec (spack.spec.Spec): Concrete spec for which build id is desired
        mirror_url (str): Base url of mirror (not including "build_cache")

    Returns:

        The cdash build id of the provided concrete spec, as an int.
    """
    if not spec.concrete:
